...
def get_train_dataflow():
    ...
    def preprocess(roidb):
        ...
        ret["orig_image_dims"] = np.asarray([[image_h, image_w, 3]])



        for i in range(len(multilevel_anchor_inputs)):
            ret[f'anchor_labels_lvl{i+2}'] = ret[f'anchor_labels_lvl{i+2}'][np.newaxis, :, :, :]
            ret[f'anchor_boxes_lvl{i+2}'] = ret[f'anchor_boxes_lvl{i+2}'][np.newaxis, :, :, :, :]
