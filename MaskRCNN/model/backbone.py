...
def resnet_shortcut(l, n_out, stride, sg, activation=tf.identity):
    n_in = l.shape[1]
    if n_in != n_out:   # change dimension when channel is not the same
        ...
        return Conv2D('convshortcut', l, n_out, 1,
                      strides=stride, activation=activation, seed=sg.next())
    else:
        return l


def resnet_bottleneck(l, ch_out, stride, sg):
    shortcut = l
    if cfg.BACKBONE.STRIDE_1X1:
        if stride == 2:
            l = l[:, :, :-1, :-1]
        l = Conv2D('conv1', l, ch_out, 1, strides=stride, seed=sg.next())
        l = Conv2D('conv2', l, ch_out, 3, strides=1, seed=sg.next())
    else:
        l = Conv2D('conv1', l, ch_out, 1, strides=1, seed=sg.next())
        if stride == 2:
    ...
    if cfg.BACKBONE.NORM != 'None':
        ...
        l = Conv2D('conv3', l, ch_out * 4, 1, activation=tf.identity,
                   kernel_initializer=tf.constant_initializer(), seed=sg.next())
    ret = l + resnet_shortcut(shortcut, ch_out * 4, stride, sg=sg, activation=get_norm(zero_init=False))
    return tf.nn.relu(ret, name='output')


def resnet_group(name, l, block_func, features, count, stride, sg):
    with tf.variable_scope(name):
        for i in range(0, count):
            with tf.variable_scope('block{}'.format(i)):
                l = block_func(l, features, stride if i == 0 else 1, sg)
    return l
...
def resnet_fpn_backbone(image, num_blocks, sg, fp16=False):
    freeze_at = cfg.BACKBONE.FREEZE_AT
