...
def fpn_model(features, sg, fp16=False):
    """
    Args:
    ...
    with mixed_precision_scope(mixed=fp16):
      with argscope(Conv2D, data_format='channels_first',
                  ...
                  kernel_initializer=tf.variance_scaling_initializer(scale=1., seed=sg.next())):
        lat_2345 = [Conv2D('lateral_1x1_c{}'.format(i + 2), c, num_channel, 1, seed=sg.next())
        ...
        for idx, lat in enumerate(lat_2345[::-1]):
            if idx == 0:
                ...
                lat_sum_5432.append(lat)
        p2345 = [Conv2D('posthoc_3x3_p{}'.format(i + 2), c, num_channel, 3, seed=sg.next())
