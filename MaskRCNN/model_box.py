...
@under_name_scope()
def crop_and_resize_batch(image, boxes, box_ind, crop_size, prepadding_img_dims, pad_border=True):
    """
    Batched aligned version of tf.image.crop_and_resize, following our definition of floating point boxes.

    Args:
        image: NCHW (note N here is batch size (BS). It is not the same as 'n' below)
        boxes: nx5, batch_idx, x1, y1, x2, y2
        box_ind: (n,)
        crop_size (int):
        prepadding_img_dims: Nx2. Original height and width of each image
