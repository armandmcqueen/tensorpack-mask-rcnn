...
class NonMaxSuppressionCustomTest : public OpsTestBase {
 protected:
  void MakeOp() {
    TF_EXPECT_OK(NodeDefBuilder("non_max_suppression_op", "NonMaxSuppressionCustom")
...
TEST_F(NonMaxSuppressionCustomTest, TestSelectFromThreeClusters) {
  MakeOp();
...
TEST_F(NonMaxSuppressionCustomTest,
...
TEST_F(NonMaxSuppressionCustomTest, TestSelectAtMostTwoBoxesFromThreeClusters) {
  MakeOp();
  AddInputFromArray<float>(
      TensorShape({6, 4}),
      {0, 0,  1, 1,  0, 0.1f,  1, 1.1f,  0, -0.1f, 1, 0.9f,
...
TEST_F(NonMaxSuppressionCustomTest, TestSelectSingleBox) {
  MakeOp();
...
TEST_F(NonMaxSuppressionCustomTest, TestSelectFromTenIdenticalBoxes) {
  MakeOp();
...
TEST_F(NonMaxSuppressionCustomTest, TestInconsistentBoxAndScoreShapes) {
  MakeOp();
  AddInputFromArray<float>(
      TensorShape({6, 4}),
      {0, 0,  1, 1,  0, 0.1f,  1, 1.1f,  0, -0.1f, 1, 0.9f,
...
TEST_F(NonMaxSuppressionCustomTest, TestInvalidIOUThreshold) {
  MakeOp();
  AddInputFromArray<float>(TensorShape({1, 4}), {0, 0, 1, 1});
  AddInputFromArray<float>(TensorShape({1}), {.9f});
  AddInputFromArray<int>(TensorShape({}), {3});
...
TEST_F(NonMaxSuppressionCustomTest, TestEmptyInput) {
  MakeOp();
